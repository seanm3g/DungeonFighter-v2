...
- **`Code/Actions/Conditional/ActionTriggerBundleApplicator.cs`** - Spreadsheet TRIGGERS triples: on matching WHEN, apply listed `→` mechanics (blank SCOPE = instant; TURN/ACTION/FIGHT/DUNGEON = lasting grant); also strip_*, retrigger_*, salvage_miss, crit_face_min, replace_next_roll (`replace_next_roll` is consumed in `ActionExecutionFlow.Selection` **before** combo-vs-normal selection so the forced face gates strip picks and amp)
