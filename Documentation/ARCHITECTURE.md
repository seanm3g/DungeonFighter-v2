...
- **Combat arena layout (mock chrome)** — Center column owns combat HUDs; combo strip sits at the **bottom** (`LayoutConstants.ACTION_INFO_Y`). `CombatArenaHudLayout` / `CombatArenaHudRenderer`: enemy HP+armor+d20 bar (top, right-aligned), fighter HP+armor+d20 bar (bottom above strip, left-aligned), split local logs via `CombatLocalLogView`. Fighter-only resolve row: `FighterResolveActionStackState` / `FighterResolveActionStackRenderer` (Previous | Current | Next). Side panels keep stats/gear/location/status (no HP/d20). Between-fight narrative paints only in `GetNarrativeBand`. Roll diamond travel: `ThresholdBarFeedback` (per-panel) + `CanvasPrimitivesRenderer.RenderRollCaret`.
