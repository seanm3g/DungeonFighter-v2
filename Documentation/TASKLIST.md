...
- **Recent**: ✅ **Action Interaction Lab — controls footer layout + Exit lab to Settings** — Footer buttons now render **Back** on the left and **Step** on the right, with blank spacer lines separating **Reset combo** and **Exit lab** for readability. **Exit lab** now always closes the lab and returns to **Settings**, force-closing and re-opening Settings to refresh the UI (`ActionLabControlsRenderer`, `GameCoordinator.ExitActionInteractionLab`). Tests: `GameCoordinatorTests`.
