...
- **Recent**: ✅ **Action Lab batch report — simulation wall time** — `RunBatchAsync` records `SimulationWallElapsed`; `FormatReportText` appends `Simulation wall time: …` at the end (`ActionLabEncounterSimulator`, tests in `ActionLabEncounterSimulatorTests`).
- **Recent**: ✅ **Action Interaction Lab — clear "Running" before results dialog** — Batch sim sets busy off and re-renders **before** opening the report modal so the footer no longer shows `Running…` behind the window (`ActionLabInputCoordinator`); same on simulation error.
- **Recent**: ✅ **Action Interaction Lab — Sim 1000 status on one line** — While a batch sim runs, `Running {n}…` draws on the same row to the right of `[ Sim 1000 ]` (narrow rows still truncate the suffix). `ActionLabControlsRenderer`.
- **Recent**: ✅ **ACTIONS sheet — SHIFT column (Jump +slots)** — CSV/sheet import and push: **`SHIFT`** maps to `SpreadsheetActionData.JumpRelative` / JSON `jumpRelative`; **`JUMP RELATIVE`** remains a fallback when **`SHIFT`** is not in the header. Serializer prefers **`SHIFT`** when present. Parser reads **`JUMP`**, **`SHIFT`** / **`JUMP RELATIVE`**, **`DISRUPT`**. Tests: `SpreadsheetActionDataSheetRowSerializerTests.TestShiftColumn_MapsJumpRelativeRoundTrip`, `TestJumpRelativeFallback_WhenNoShiftColumn`. Files: `SpreadsheetActionDataSheetRowSerializer.cs`, `SpreadsheetActionDataCsvParser.cs`, `SpreadsheetActionData.cs`.
- **Manual (Google Sheet, not in repo)**: Reorder **ACTIONS** so the column that was at **AJ** sits next to **WEAPON SPEED** / **WEAPON DAMAGE** (move header + data together; avoid duplicate row-2 labels). Rename/insert mechanics headers to **`JUMP`**, **`SHIFT`**, **`DISRUPT`** (drop **`JUMP RELATIVE`** when migrated).
- **Recent**: ✅ **Action Interaction Lab — hero HP bar clicks** — Left-click on the hero health bar reduces HP by **5% of effective max** per click; right-click restores **+5** HP (capped at max). Helpers: `ActionLabLeftPanelStatAdjustment.ApplyHeroHpClickDamagePercent` / `ApplyHeroHpRightClickHeal`; wiring in `MouseInteractionHandler`. Tests: `ActionInteractionLabTests.LeftPanelStatAdjustment_HeroHpDamageAndHeal`.
- **Recent**: ✅ **Action Interaction Lab — default d20 selection** — Opening the Action Lab now starts in fixed mode with **`d20 sel: 16`** by default (`ActionInteractionLabSession.SelectedD20`). Tests: `ActionInteractionLabTests.LabSessionBeginsWithDefaultD20Selection16`.
- **Recent**: ✅ **Sheets push/pull — enemies & environments** — Optional tabs **ENEMIES** → `Enemies.json` and **ENVIRONMENTS** → `Rooms.json`: `SheetsPushConfig` / `SheetsConfig` fields, `GameDataTabularSheetKind` + `JsonArraySheetConverter` canonical columns, `GameDataSheetsPushService` / `GameDataSheetsPullService`, Balance Tuning panel gid boxes, `GoogleSheetsUrlHelper` id sync. Tests: `JsonArraySheetConverterTests`, `SheetsPushConfigTests`.
- **Recent**: ✅ **AMP / INT accuracy vs tuning JSON** — `TuningConfig.json` had `intelligenceRollBonusPer: 0` (INT gave no roll bonus) and `comboSystem` max/maxTech at 0 (TECH clamped so AMP stayed 1.0x). Fix: `AttributesConfig.EnsureValidIntelligenceRollBonusDefaults()` (default 10), `ComboSystemConfig.EnsureValidComboAmplifierDefaults()` (uses `GameConstants` max / max TECH), called from `GameConfiguration.LoadFromFile`; JSON updated. Tests: `CharacterConfigTests`, `CombatConfigTests`.
- **Recent**: ✅ **Action Lab / combat — TECH & INT stat bonuses** — Spreadsheet and execution code queue bonuses as `TECH` (and sometimes `INTELLIGENCE`), but `CharacterStats.ApplyStatBonus` only handled `TEC` / `INT`, so temporary technique and intelligence bonuses from actions never applied (including in the Action Interaction Lab). Fixed: accept `TECH`, `TECHNIQUE`, `STRENGTH`, `AGILITY`, `INTELLIGENCE`; aligned `ApplyPreRollBonuses` next-attack lookup. Tests: `CharacterAttributesTests.TestApplyStatBonusAliasesMatchSpreadsheetAndActions`.
...
- **Recent**: ✅ **Action Interaction Lab — controls footer layout + Exit lab to Settings** — Footer buttons now render **Back** on the left and **Step** on the right, with blank spacer lines separating **Reset** and **Exit lab** for readability. **Exit lab** now always closes the lab and returns to **Settings**, force-closing and re-opening Settings to refresh the UI (`ActionLabControlsRenderer`, `GameCoordinator.ExitActionInteractionLab`). Tests: `GameCoordinatorTests`.
...
**Tasks**:
- [x] Define `Regions.json` and 100 travel event entries in `TravelEvents.json`
- [x] Persist each character's current region
- [x] Add travel menu, handler, renderer, and input routing
- [x] Generate 10 route events per destination using combat-style d20 outcome bands
- [x] Filter dungeon selection by current region
- [x] Add focused tests for travel data, rolls, persistence, UI/input, and dungeon filtering
