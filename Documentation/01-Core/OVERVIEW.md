...
- **Action Combo System:** Chain actions together for increased damage and effects (1.85x damage multiplier per combo step). Combo routing supports an absolute **Jump** (target slot) and a **Jump (+slots)** field (`jumpRelative` in JSON): extra steps beyond the usual next action from the current combo index (e.g. combo position 2 with +1 lands on the fourth action). On the **ACTIONS** Google Sheet, **Jump (+slots)** round-trips as the **`SHIFT`** column (between **`JUMP`** and **`DISRUPT`**); legacy sheets may still use **`JUMP RELATIVE`**, which the importer and push serializer also accept when **`SHIFT`** is absent.
...
- **Combat action strip:** Per-combo-slot cards show damage and speed; when total roll bonus for that slot is positive (same basis as tooltip accuracy), the card also shows **`Acc +N`** in green.

### Data-driven Architecture
- **JSON Configuration:** All game data stored in structured JSON files
- **Google Sheets ACTIONS tab:** Column layout is label-driven (row 1 context, row 2 headers). **`WEAPON SPEED`** / **`WEAPON DAMAGE`** (and adjacent author columns) are not mapped to dedicated C# fields unless added later; keep row-1 merges and unique labels so `SpreadsheetHeader` resolves the correct cells. Next-action **SpeedMod / DamageMod / MultiHitMod / AmpMod** map from row-1 **`HERO BASE STATS`** (preferred) or legacy **`ENEMY BASE STATS`**, using column labels **`SPEED MOD`** / **`DAMAGE MOD`** or the common aliases **`ACTION SPEED`** / **`ACTION DAMAGE`** (line breaks inside header cells are normalized for matching). Combo mechanics: **`JUMP`**, **`SHIFT`** (Jump +slots / `jumpRelative`), **`DISRUPT`**. Optional row-2 **`TAGS`** column: comma/semicolon-separated extra tokens (imported by `SpreadsheetActionDataCsvParser`); **category** and **rarity** still merge into runtime tags separately. **Weapons / armor / enemies** sheets support optional **`tags`** as JSON string arrays in cells for `JsonArraySheetConverter` round-trip.
- **Mod-trade action catalog:** Review list of hero/enemy action patterns built from next-action mod trades (same-character, dual-sided same-stat, and 64 cross-stat quadrants) with stable `PatternId`s and sheet column hints — see [ModTradeActionCatalog.md](ModTradeActionCatalog.md). Runtime data: mod-trade rows live in `GameData/Actions.json` with **`tags`:** `modtrade` and **`category`:** `ModTrade` (regenerated by `scripts/generate_mod_trade_actions.py`).
