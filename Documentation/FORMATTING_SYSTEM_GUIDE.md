...
- **Live canvas sequence HUD:** combat-only chrome under the action strip (`CombatSequenceHudState.ShouldReserveBand` for Combat and Action Lab; never Skill Tree / dungeon / hub). Combat log order is **setup** (`{Actor} Attacks {Target}...`) → HUD playback → **punchline** (`... and hits` / `... and misses`). `CombatSequencePresenter.PlayPendingOrWaitAsync` sits between those beats. Columns: ATTACKER / ROLL / OUTCOME / ACTION / DEFENSE / DAMAGE / EFFECTS in a two-row framed panel with a one-row gap above the combat log (`LayoutConstants.COMBAT_SEQUENCE_HUD_HEIGHT`, `COMBAT_SEQUENCE_LOG_GAP`). Headers stay visible with no swing in progress during Combat. All steps sit **horizontally** (titles on row 1, results on row 2); the active column is gold and bracketed while its formula pieces play in order (`CombatSequenceMathBeats`). ATTACKER is the actor name; the named ability waits until ACTION after OUTCOME. Unnamed synthetic normals show **hit** or **miss** in ACTION (not “unnamed hit”). HUD paints are posted to the UI thread via `ForceRender` (combat-thread `InvalidateVisual` freezes the canvas). Instant, muted, and console paths skip the HUD and keep the half-delay setup/punchline. HUD beat gap is `MessageDelayMs × 1.5` (`SequenceHudDelayMultiplier`). HP bars use `HealthBarDisplayHold` until the DAMAGE/HEAL cue.
