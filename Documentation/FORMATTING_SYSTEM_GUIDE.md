...
- **Live canvas sequence HUD:** dungeon chrome at the top of the center column (`CombatSequenceHudState.ShouldReserveBand` for Dungeon and Combat); fighter combo cards stay at the bottom. Combat log order is **setup** (`{Actor} Attacks {Target}...`) → HUD playback → **punchline** (`... and hits` / `... and misses`). `CombatSequencePresenter.PlayPendingOrWaitAsync` sits between those beats. Columns: ATTACKER / ROLL / OUTCOME / ACTION / DEFENSE / DAMAGE / EFFECTS in a two-row framed panel with a one-row gap above the combat log (`LayoutConstants.COMBAT_SEQUENCE_HUD_HEIGHT`, `COMBAT_SEQUENCE_LOG_GAP`). Headers stay visible with no swing in progress. All steps sit **horizontally** (titles on row 1, results on row 2); the active column is gold and bracketed while its formula pieces play in order (`CombatSequenceMathBeats`). ATTACKER is the actor name; the named ability waits until ACTION after OUTCOME. A plain hit shows **N/A** in ACTION (no strip action used); unnamed misses still show **miss**. Combo and crit show the action name. HUD paints are posted to the UI thread via `ForceRender` (combat-thread `InvalidateVisual` freezes the canvas). Instant, muted, and console paths skip the HUD and keep the half-delay setup/punchline. HUD beat gap is `MessageDelayMs × 1.5` (`SequenceHudDelayMultiplier`). HP bars use `HealthBarDisplayHold` until the DAMAGE/HEAL cue. The centered resolve card (`FighterResolveActionStackRenderer`) gets a red X overlay when the swing does not use the action (miss, fail, or plain hit). Combo and crit leave the card unmarked; archived left/right past cards keep that mark.
...
- `Documentation/COMBAT_LOG_SPACING_STANDARD.md`
- `Documentation/FORMATTING_SYSTEM_GUIDE.md` (this document)
...
- **Combat log spacing:** `Documentation/COMBAT_LOG_SPACING_STANDARD.md`
