...
**Spawn is not this lookup.** `RoomGenerator` uses `RoomLoader.GetRoomsByTheme` (blank biome = universal). Flavor can disagree with `Environment.Theme` (Sand Dune in Crypt still has dungeon theme Crypt, flavor biome `Desert` via `dune`). Blank-biome universal catalog is a Max/Sean architecture sign-off, not a flavor-content fix.
