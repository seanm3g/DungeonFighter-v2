...
namespace RPGGame
{
    ...
    public static class SkillTreeService
    {
            ...
            bool rebuildActions = true)
        {
            ...
            int rankBefore = character.Progression.GetSkillRank(nodeId);
            ...
            if (rankBefore < 1)
                ApplyRiteBonusOnFirstLearn(character, nodeId);
        ...
        /// <summary>
        /// Rite of passage nodes grant a permanent +15 primary stat on first learn (persists via base stats in save).
        /// </summary>
        public static void ApplyRiteBonusOnFirstLearn(Character character, string nodeId)
        {
            var node = Trees.GetNode(nodeId);
            if (node == null || string.IsNullOrWhiteSpace(node.CustomEffectId))
                return;

            switch (node.CustomEffectId.Trim().ToLowerInvariant())
            {
                case "puberty":
                    character.Stats.Strength += 15;
                    break;
                case "commission":
                    character.Stats.Agility += 15;
                    break;
                case "initiation":
                    character.Stats.Technique += 15;
                    break;
                case "apprenticeship":
                    character.Stats.Intelligence += 15;
                    break;
            }
        }
