...
namespace RPGGame
{
    ...
    public sealed class SkillEffectRouter
    {
        ...
        private int _bankedArmor;
        private int _bankedSpeed;
        private int _bankedDamage;
        private int _bankedAmp;
        private bool _avengeWarriorTagsGranted;
        private int _ironHideArmorApplied;
        private bool _daggerCritMissRerollUsed;
        ...
        public void ResetFightState()
        {
            ...
            _bankedDamage = 0;
            _bankedAmp = 0;
            _avengeWarriorTagsGranted = false;
            _ironHideArmorApplied = 0;
            _daggerCritMissRerollUsed = false;
            CombatTriggerContext.ClearFightGrantedActionTags();
        }
        ...
        public int GetEffectRank(Character? c, string customEffectId) => GetRank(c, customEffectId);

        public bool HasEffect(Character? c, string customEffectId) => Has(c, customEffectId);
        ...
        /// <summary>Flat armor from Bone Temper while Harden is active, plus iron_hide_roll bank.</summary>
        public int GetSkillArmorBonus(Character? c)
        {
            if (c == null) return 0;
            int bonus = 0;
            int bone = GetRank(c, "bone_temper");
            if (bone > 0 && (c.HardenStacks ?? 0) > 0)
                bonus += bone;
            if (_ironHideArmorApplied > 0 && ReferenceEquals(c, _boundCharacter))
                bonus += _ironHideArmorApplied;
            return bonus;
        }

        /// <summary>
        /// Applies learned cadence rules (+1 combo ease per 2 class tags) at fight start.
        /// </summary>
        public void ApplyCadenceThresholds(Character hero, global::RPGGame.Combat.ThresholdManager thresholdManager)
        {
            if (hero == null || thresholdManager == null) return;
            RefreshForCharacter(hero);
            TryCadence(hero, thresholdManager, "heavy_cadence", ClassMaterialTagAlias.Barbarian);
            TryCadence(hero, thresholdManager, "drill_cadence", ClassMaterialTagAlias.Warrior);
            TryCadence(hero, thresholdManager, "soft_cadence", ClassMaterialTagAlias.Rogue);
            TryCadence(hero, thresholdManager, "soft_cadence_wiz", ClassMaterialTagAlias.Wizard);
        }

        private void TryCadence(Character hero, global::RPGGame.Combat.ThresholdManager tm, string effectId, string classTag)
        {
            if (!Has(hero, effectId)) return;
            int tags = ClassMaterialTagAlias.CountEquippedClassTags(hero, classTag);
            int ease = tags / 2;
            if (ease > 0)
                tm.AdjustComboThreshold(hero, ease);
        }

        /// <summary>Consume banked next-turn bonuses at the start of a hero swing.</summary>
        public void ApplyBankedRollBonuses(Character hero)
        {
            if (hero == null || !ReferenceEquals(hero, _boundCharacter)) return;

            // Previous turn's iron hide expires when the next swing starts.
            _ironHideArmorApplied = 0;

            if (_bankedSpeed > 0 && Has(hero, "roll_speed"))
            {
                hero.Effects.AddPendingActionBonusesNextHeroRoll(new List<ActionAttackBonusItem>
                {
                    new() { Type = "SPEED_MOD", Value = _bankedSpeed }
                });
                _bankedSpeed = 0;
            }

            if (_bankedDamage > 0 && Has(hero, "roll_damage"))
            {
                hero.Effects.AddPendingActionBonusesNextHeroRoll(new List<ActionAttackBonusItem>
                {
                    new() { Type = "DAMAGE_MOD", Value = _bankedDamage }
                });
                _bankedDamage = 0;
            }

            if (_bankedAmp > 0 && Has(hero, "roll_amp"))
            {
                hero.Effects.ConsumedAmpModPercent += _bankedAmp;
                _bankedAmp = 0;
            }
        }

        /// <summary>Activates banked iron_hide armor after the swing that rolled it (covers the following turn).</summary>
        public void ActivatePendingIronHideArmor(Character hero)
        {
            if (hero == null || !ReferenceEquals(hero, _boundCharacter)) return;
            if (_bankedArmor <= 0 || !Has(hero, "iron_hide_roll")) return;
            _ironHideArmorApplied = _bankedArmor;
            _bankedArmor = 0;
        }

        /// <summary>True when dagger_critmiss_reroll should force one crit-miss reroll this fight.</summary>
        public bool TryConsumeDaggerCritMissReroll(Character hero)
        {
            if (hero == null || _daggerCritMissRerollUsed) return false;
            if (!Has(hero, "dagger_critmiss_reroll")) return false;
            if (hero.Equipment.Weapon is not WeaponItem w || w.WeaponType != WeaponType.Dagger)
                return false;
            _daggerCritMissRerollUsed = true;
            return true;
        }
        ...
        private void OnMiss(CombatEvent evt)
        {
            ...
            BankNaturalRoll(hero, evt.NaturalRollValue);
        ...
        private void OnHit(CombatEvent evt)
        {
            ...
            ApplyConcussiveCrit(hero, evt);
            ApplyPrecisionMultihit(hero, evt);
            ApplyAvengeWarriorTags(hero, evt);
            ApplyConcentratedChaos(hero, evt);
        ...
        private void BankNaturalRoll(Character hero, int naturalRoll)
        {
            if (naturalRoll <= 0) return;
            if (Has(hero, "iron_hide_roll"))
                _bankedArmor = naturalRoll;
            if (Has(hero, "roll_speed"))
                _bankedSpeed = naturalRoll;
            if (Has(hero, "roll_damage"))
                _bankedDamage = naturalRoll;
            if (Has(hero, "roll_amp"))
                _bankedAmp = naturalRoll;
        }
        ...
        {
            var heroSource = AsBoundHero(evt.Source);
            if (heroSource != null)
                ActivatePendingIronHideArmor(heroSource);
        ...
        private void ApplyBronzeAges(Character hero, CombatEvent evt)
        {
            ...
            int barbItems = ClassMaterialTagAlias.CountEquippedClassTags(hero, ClassMaterialTagAlias.Barbarian);
        ...
        private void ApplyFirstFormation(Character hero, CombatEvent evt)
        {
            ...
            int items = ClassMaterialTagAlias.CountEquippedClassTags(hero, ClassMaterialTagAlias.Warrior);
            if (items <= 0) return;
        ...
        private void ApplyFirstCut(Character hero, CombatEvent evt)
        {
            ...
            if (rank <= 0 || evt.Action == null) return;
            int items = ClassMaterialTagAlias.CountEquippedClassTags(hero, ClassMaterialTagAlias.Rogue);
            if (items <= 0) return;
        ...
        private void ApplyFirstGlyph(Character hero, CombatEvent evt)
        {
            ...
            if (rank <= 0 || evt.Action == null) return;
            int items = ClassMaterialTagAlias.CountEquippedClassTags(hero, ClassMaterialTagAlias.Wizard);
            if (items <= 0) return;
        ...
        private void ApplyConcussiveCrit(Character hero, CombatEvent evt)
        {
            if (!Has(hero, "concussive_crit") || evt.Target == null || !evt.IsCritical)
                return;
            if (hero.Equipment.Weapon is not WeaponItem w || w.WeaponType != WeaponType.Mace)
                return;
            evt.Target.IsStunned = true;
            evt.Target.StunTurnsRemaining = Math.Max(evt.Target.StunTurnsRemaining, 1);
        }

        private void ApplyPrecisionMultihit(Character hero, CombatEvent evt)
        {
            if (!Has(hero, "precision_multihit") || evt.Action == null) return;
            if (!ActionTriggerPredicates.SubjectHasTag(hero, evt.Action, ClassMaterialTagAlias.Rogue))
                return;
            hero.Effects.ConsumedMultiHitMod += 1;
        }

        private void ApplyAvengeWarriorTags(Character hero, CombatEvent evt)
        {
            if (evt.Action == null) return;
            if (!string.Equals(evt.Action.Name, "AVENGE", StringComparison.OrdinalIgnoreCase))
                return;
            if (!Has(hero, "avenge") && !SkillTreeService.IsTreeActionName("AVENGE"))
                return;
            if (_avengeWarriorTagsGranted) return;
            _avengeWarriorTagsGranted = true;
            CombatTriggerContext.GrantFightActionTag(hero, ClassMaterialTagAlias.Warrior);
        }

        private void ApplyConcentratedChaos(Character hero, CombatEvent evt)
        {
            if (evt.Action == null) return;
            if (!string.Equals(evt.Action.Name, "CONCENTRATED CHAOS", StringComparison.OrdinalIgnoreCase))
                return;
            if (!Has(hero, "concentrated_chaos")) return;
            var strip = CombatTriggerContext.GetStripState(hero);
            int skipped = strip?.SkippedSlotsPassed ?? 0;
            int disabled = strip?.DisabledSlots.Count ?? 0;
            int n = skipped + disabled;
            if (n <= 0) return;
            AddDamageMod(hero, 35.0 * n);
        }
