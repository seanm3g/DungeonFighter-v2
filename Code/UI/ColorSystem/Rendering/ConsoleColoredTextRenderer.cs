...
namespace RPGGame.UI.ColorSystem
{
    ...
    public class ConsoleColoredTextRenderer : IColoredTextRenderer
    {
            ...
            foreach (var segment in segments)
            {
                ...
                if (segment.Text.Contains("\r\n") || segment.Text.Contains("\n") || segment.Text.Contains("\r"))
                {
                    ...
                    for (int i = 0; i < newlineSplit.Length; i++)
                    {
                        ...
                        {
                            ProcessSegmentPartForConsole(part, segment.Color, ref currentLine, ref currentLength, maxWidth, ref lines);
                        }
                    }
        ...
        /// <summary>
        /// Helper method to process a segment part (after splitting on newlines) for Console renderer
        /// Handles word wrapping for the part
        /// </summary>
        private void ProcessSegmentPartForConsole(string part, global::Avalonia.Media.Color color, ref List<ColoredText> currentLine, ref int currentLength, int maxWidth, ref List<List<ColoredText>> lines)
        {
