...
namespace RPGGame.UI.Avalonia.Managers.Settings.PanelHandlers
{
    ...
    public sealed class EnemyTuningPanelHandler : ISettingsPanelHandler
    {
        ...
        public void WireUp(UserControl panel)
        {
            ...
            {
                if (c?.Slider != null)
                    c.Slider.ValueChanged += (_, _) => PushProgressionScalesFromPanel(p);
            }
            ...
            if (baseHealth?.TextBox != null)
                baseHealth.TextBox.LostFocus += (_, _) => PushProgressionScalesFromPanel(p);
            if (healthGrowth?.TextBox != null)
                healthGrowth.TextBox.LostFocus += (_, _) => PushProgressionScalesFromPanel(p);
            if (attributeGrowth?.TextBox != null)
                attributeGrowth.TextBox.LostFocus += (_, _) => PushProgressionScalesFromPanel(p);

            foreach (var box in SpawnWeightFields.AllTextBoxes(p))
            {
                if (box == null) continue;
                box.TextChanged += (_, _) => UpdateTotalLabels(p);
                box.LostFocus += (_, _) => PushSpawnWeightsFromPanel(p);
            }

            Dispatcher.UIThread.Post(() =>
            {
                LoadSettings(p);
                SyncSliderDisplays(p);
        ...
        public void SaveSettings(UserControl panel)
        {
            ...
            try
            {
                PushProgressionScalesFromPanel(p);
                PushSpawnWeightsFromPanel(p);
                GameConfiguration.Instance.EnemySystem?.EnsureSanitizedDefaults();
                LoadSettings(p);
        ...
        private static void SyncSliderDisplays(EnemyTuningSettingsPanel p)
        {
            ResolveSlider(p, p.EnemyBaseHealthScaleControl, "EnemyBaseHealthScaleControl")?.SyncDisplayFromValue();
            ResolveSlider(p, p.EnemyHealthGrowthScaleControl, "EnemyHealthGrowthScaleControl")?.SyncDisplayFromValue();
            ResolveSlider(p, p.EnemyAttributeGrowthScaleControl, "EnemyAttributeGrowthScaleControl")?.SyncDisplayFromValue();
        }

        private static void PushProgressionScalesFromPanel(EnemyTuningSettingsPanel p)
        {
        ...
        private static void PushSpawnWeightsFromPanel(EnemyTuningSettingsPanel p)
        {
            var sys = GameConfiguration.Instance.EnemySystem ??= new EnemySystemConfig();
        ...
        private static void UpdateTotalLabel(TextBlock? label, int total)
        {
            ...
            label.Classes.Clear();
            label.Classes.Add(total == 100 ? "settings-muted" : "settings-total-warning");
        }

        internal static class SpawnWeightFields
        {
