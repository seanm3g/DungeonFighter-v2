...
namespace RPGGame.UI.Avalonia.Display
{
    ...
    public class DisplayRenderer
    {
        ...
        public void Render(DisplayBuffer buffer, int contentX, int contentY, int contentWidth, int contentHeight, bool clearContent = true, IReadOnlyList<string>? combatEnemyNamesForPrimaryLineRightAlign = null, string? combatHeroNameForLineAlignment = null)
        {
            ...
            // Do not extend the clear band into the action-info strip (bottom band under framed center content).
            if (clearContent)
            {
                int scrollOverflowPad = Math.Max(0, contentY - 2);
                int stripTop = LayoutConstants.ACTION_INFO_Y;
                int clearStartY = scrollOverflowPad;
                int clearEndY = Math.Min(contentY + contentHeight, stripTop);
