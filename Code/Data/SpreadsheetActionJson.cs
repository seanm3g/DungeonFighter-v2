using System;
using System.Text.Json.Serialization;

namespace RPGGame.Data
{
    /// <summary>
    /// JSON model that matches Google Sheets column structure exactly.
    /// All values are preserved as strings to match spreadsheet format.
    /// Uses custom converter to omit empty string properties for efficiency.
    /// Properties are split across partials by column group: Core, Stats, StatusEffects, Mechanics, Triggers, Thresholds.
    /// </summary>
    [JsonConverter(typeof(SpreadsheetActionJsonConverter))]
    public partial class SpreadsheetActionJson
    {
        /// <summary>
        /// Converts SpreadsheetActionData to SpreadsheetActionJson
        /// </summary>
        public static SpreadsheetActionJson FromSpreadsheetActionData(SpreadsheetActionData data)
        {
            return new SpreadsheetActionJson
            {
                Action = data.Action,
                Description = data.Description,
                ColumnC = data.ColumnC,
                Rarity = data.Rarity,
                Category = data.Category,
                Tier = data.Tier,
                DPS = data.DPS,
                NumberOfHits = data.NumberOfHits,
                Damage = data.Damage,
                Speed = data.Speed,
                Energy = data.Energy,
                Duration = data.Duration,
                Cadence = data.Cadence,
                Mechanics = data.Mechanics,
                CadenceDuration = data.CadenceApplicationCount,
                CadenceBundlesJson = data.CadenceBundlesJson ?? "",
                Opener = data.Opener,
                Finisher = data.Finisher,
                ReservePool = data.ReservePool,
                MaterialScale = data.MaterialScale,
                KeywordScale = data.KeywordScale,
                ScaleFormula = data.ScaleFormula,
                HeroAccuracy = data.HeroAccuracy,
                HeroHit = data.HeroHit,
                HeroCombo = data.HeroCombo,
                HeroCrit = data.HeroCrit,
                HeroCritMiss = data.HeroCritMiss,
                EnemyAccuracy = data.EnemyAccuracy,
                EnemyHit = data.EnemyHit,
                EnemyCombo = data.EnemyCombo,
                EnemyCrit = data.EnemyCrit,
                EnemyCritMiss = data.EnemyCritMiss,
                HeroSTR = data.HeroSTR,
                HeroAGI = data.HeroAGI,
                HeroTECH = data.HeroTECH,
                HeroINT = data.HeroINT,
                EnemySTR = data.EnemySTR,
                EnemyAGI = data.EnemyAGI,
                EnemyTECH = data.EnemyTECH,
                EnemyINT = data.EnemyINT,
                SpeedMod = data.SpeedMod,
                DamageMod = data.DamageMod,
                MultiHitMod = data.MultiHitMod,
                AmpMod = data.AmpMod,
                EnemySpeedMod = data.EnemySpeedMod,
                EnemyDamageMod = data.EnemyDamageMod,
                EnemyMultiHitMod = data.EnemyMultiHitMod,
                EnemyAmpMod = data.EnemyAmpMod,
                WeaponSpeedMod = data.WeaponSpeedMod,
                WeaponDamageMod = data.WeaponDamageMod,
                EnemyWeaponSpeedMod = data.EnemyWeaponSpeedMod,
                EnemyWeaponDamageMod = data.EnemyWeaponDamageMod,
                Stun = data.Stun,
                Poison = data.Poison,
                Burn = data.Burn,
                Bleed = data.Bleed,
                Weaken = data.Weaken,
                Expose = data.Expose,
                Slow = data.Slow,
                Vulnerability = data.Vulnerability,
                Harden = data.Harden,
                Silence = data.Silence,
                Pierce = data.Pierce,
                StatDrain = data.StatDrain,
                Fortify = data.Fortify,
                Consume = data.Consume,
                Focus = data.Focus,
                Cleanse = data.Cleanse,
                Lifesteal = data.Lifesteal,
                Confuse = data.Confuse,
                Reflect = data.Reflect,
                SelfDamage = data.SelfDamage,
                HeroHeal = data.HeroHeal,
                HeroHealMaxHealth = data.HeroHealMaxHealth,
                ReplaceNextRoll = data.ReplaceNextRoll,
                HighestLowestRoll = data.HighestLowestRoll,
                DiceRolls = data.DiceRolls,
                ExplodingDiceThreshold = data.ExplodingDiceThreshold,
                Curse = data.Curse,
                Skip = data.Skip,
                Jump = data.Jump,
                JumpRelative = data.JumpRelative,
                Disrupt = data.Disrupt,
                Grace = data.Grace,
                LoopChain = data.LoopChain,
                Shuffle = data.Shuffle,
                ReplaceAction = data.ReplaceAction,
                ChainLength = data.ChainLength,
                ChainPosition = data.ChainPosition,
                ModifyBasedOnChainPosition = data.ModifyBasedOnChainPosition,
                DistanceFromXSlot = data.DistanceFromXSlot,
                OnHit = data.OnHit,
                OnMiss = data.OnMiss,
                OnCrit = data.OnCrit,
                OnKill = data.OnKill,
                OnRoomsCleared = data.OnRoomsCleared,
                OnRollValue = data.OnRollValue,
                TriggerConditions = data.TriggerConditions ?? "",
                TriggerBundlesJson = data.TriggerBundlesJson ?? "",
                StatBonusesJson = data.StatBonusesJson ?? "",
                ThresholdsJson = data.ThresholdsJson ?? "",
                AccumulationsJson = data.AccumulationsJson ?? "",
                ChainPositionBonusesJson = data.ChainPositionBonusesJson ?? "",
                ActionAttackBonusesJson = data.ActionAttackBonusesJson ?? "",
                Target = data.Target,
                ThresholdCategory = data.ThresholdCategory,
                ThresholdAmount = data.ThresholdAmount,
                Bonus = data.Bonus,
                BonusAttribute = data.BonusAttribute,
                Value = data.Value,
                Attribute = data.Attribute,
                Reset = data.Reset,
                ResetBlockerBuffer = data.ResetBlockerBuffer,
                ModifyRoom = data.ModifyRoom,
                Tags = data.Tags,
                IsDefaultAction = data.IsDefaultAction ?? "",
                WeaponTypes = data.WeaponTypes ?? ""
            };
        }

        /// <summary>
        /// Converts SpreadsheetActionJson to SpreadsheetActionData
        /// </summary>
        public SpreadsheetActionData ToSpreadsheetActionData()
        {
            return new SpreadsheetActionData
            {
                Action = this.Action,
                Description = this.Description,
                ColumnC = this.ColumnC,
                Rarity = this.Rarity,
                Category = this.Category,
                Tier = this.Tier,
                DPS = this.DPS,
                NumberOfHits = this.NumberOfHits,
                Damage = this.Damage,
                Speed = this.Speed,
                Energy = this.Energy,
                Duration = this.Duration,
                Cadence = this.Cadence,
                Mechanics = this.Mechanics,
                CadenceApplicationCount = this.CadenceDuration,
                CadenceBundlesJson = this.CadenceBundlesJson ?? "",
                Opener = this.Opener,
                Finisher = this.Finisher,
                ReservePool = this.ReservePool,
                MaterialScale = this.MaterialScale,
                KeywordScale = this.KeywordScale,
                ScaleFormula = this.ScaleFormula,
                HeroAccuracy = this.HeroAccuracy,
                HeroHit = this.HeroHit,
                HeroCombo = this.HeroCombo,
                HeroCrit = this.HeroCrit,
                HeroCritMiss = this.HeroCritMiss,
                EnemyAccuracy = this.EnemyAccuracy,
                EnemyHit = this.EnemyHit,
                EnemyCombo = this.EnemyCombo,
                EnemyCrit = this.EnemyCrit,
                EnemyCritMiss = this.EnemyCritMiss,
                HeroSTR = this.HeroSTR,
                HeroAGI = this.HeroAGI,
                HeroTECH = this.HeroTECH,
                HeroINT = this.HeroINT,
                EnemySTR = this.EnemySTR,
                EnemyAGI = this.EnemyAGI,
                EnemyTECH = this.EnemyTECH,
                EnemyINT = this.EnemyINT,
                SpeedMod = this.SpeedMod,
                DamageMod = this.DamageMod,
                MultiHitMod = this.MultiHitMod,
                AmpMod = this.AmpMod,
                EnemySpeedMod = this.EnemySpeedMod,
                EnemyDamageMod = this.EnemyDamageMod,
                EnemyMultiHitMod = this.EnemyMultiHitMod,
                EnemyAmpMod = this.EnemyAmpMod,
                WeaponSpeedMod = this.WeaponSpeedMod,
                WeaponDamageMod = this.WeaponDamageMod,
                EnemyWeaponSpeedMod = this.EnemyWeaponSpeedMod,
                EnemyWeaponDamageMod = this.EnemyWeaponDamageMod,
                Stun = this.Stun,
                Poison = this.Poison,
                Burn = this.Burn,
                Bleed = this.Bleed,
                Weaken = this.Weaken,
                Expose = this.Expose,
                Slow = this.Slow,
                Vulnerability = this.Vulnerability,
                Harden = this.Harden,
                Silence = this.Silence,
                Pierce = this.Pierce,
                StatDrain = this.StatDrain,
                Fortify = this.Fortify,
                Consume = this.Consume,
                Focus = this.Focus,
                Cleanse = this.Cleanse,
                Lifesteal = this.Lifesteal,
                Confuse = this.Confuse,
                Reflect = this.Reflect,
                SelfDamage = this.SelfDamage,
                HeroHeal = this.HeroHeal,
                HeroHealMaxHealth = this.HeroHealMaxHealth,
                ReplaceNextRoll = this.ReplaceNextRoll,
                HighestLowestRoll = this.HighestLowestRoll,
                DiceRolls = this.DiceRolls,
                ExplodingDiceThreshold = this.ExplodingDiceThreshold,
                Curse = this.Curse,
                Skip = this.Skip,
                Jump = this.Jump,
                JumpRelative = this.JumpRelative,
                Disrupt = this.Disrupt,
                Grace = this.Grace,
                LoopChain = this.LoopChain,
                Shuffle = this.Shuffle,
                ReplaceAction = this.ReplaceAction,
                ChainLength = this.ChainLength,
                ChainPosition = this.ChainPosition,
                ModifyBasedOnChainPosition = this.ModifyBasedOnChainPosition,
                DistanceFromXSlot = this.DistanceFromXSlot,
                OnHit = this.OnHit,
                OnMiss = this.OnMiss,
                OnCrit = this.OnCrit,
                OnKill = this.OnKill,
                OnRoomsCleared = this.OnRoomsCleared,
                OnRollValue = this.OnRollValue,
                TriggerConditions = this.TriggerConditions ?? "",
                TriggerBundlesJson = this.TriggerBundlesJson ?? "",
                StatBonusesJson = this.StatBonusesJson ?? "",
                ThresholdsJson = this.ThresholdsJson ?? "",
                AccumulationsJson = this.AccumulationsJson ?? "",
                ChainPositionBonusesJson = this.ChainPositionBonusesJson ?? "",
                ActionAttackBonusesJson = this.ActionAttackBonusesJson ?? "",
                Target = this.Target,
                ThresholdCategory = this.ThresholdCategory,
                ThresholdAmount = this.ThresholdAmount,
                Bonus = this.Bonus,
                BonusAttribute = this.BonusAttribute,
                Value = this.Value,
                Attribute = this.Attribute,
                Reset = this.Reset,
                ResetBlockerBuffer = this.ResetBlockerBuffer,
                ModifyRoom = this.ModifyRoom,
                Tags = this.Tags,
                IsDefaultAction = this.IsDefaultAction ?? "",
                WeaponTypes = this.WeaponTypes ?? ""
            };
        }
    }
}
