...
{
    /// <summary>Pushes Actions plus optional WEAPONS / MODIFICATIONS / ARMOR / CLASSES tabs via Sheets API.</summary>
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (cfg.ApplyDefaultOptionalSheetTabNamesWhenAllUnset())
            {
                try
                {
                    ...
                    result.AddLine(
                        "Set default push tab names in SheetsPushConfig.json: WEAPONS, MODIFICATIONS, ARMOR, CLASSES " +
                        "(all four were blank). Edit the file if your sheet uses different tab titles.");
                }
                catch (Exception ex)
                {
