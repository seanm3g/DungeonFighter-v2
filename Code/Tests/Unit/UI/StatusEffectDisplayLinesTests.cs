...
namespace RPGGame.Tests.Unit.UI
{
    public static class StatusEffectDisplayLinesTests
    {
        public static void RunAllTests()
        {
            ...
            var clean = TestDataBuilders.Character().WithName("CleanHero").Build();
            var noEffects = StatusEffectDisplayLines.Build(clean, clean);
            TestBase.AssertEqual(0, noEffects.Count, "no effects yields empty list", ref run, ref passed, ref failed);
