...
{
    /// <summary>
    /// Converts effective TECH into per-row HIT/COMBO/CRIT threshold improvements based on the tuning table.
    /// Applies these as threshold shifts (not roll bonus) so they affect hit/combo/crit bands correctly.
    /// </summary>
    public static class TechniqueMilestoneThresholdBonuses
    {
        private readonly record struct Milestone(int TechniqueThreshold, int Hit, int Combo, int Crit);
        ...
        public static (int HitSteps, int ComboSteps, int CritSteps) GetSteps(int effectiveTechnique)
        {
            int hit = 0, combo = 0, crit = 0;
            for (int i = 0; i < Table.Length; i++)
            {
                if (effectiveTechnique < Table[i].TechniqueThreshold)
                    break;
                hit += Table[i].Hit;
                combo += Table[i].Combo;
                crit += Table[i].Crit;
        ...
        /// <summary>
        /// Applies TECH milestone deltas to the actor's thresholds.
        ...
        {
            int effectiveTechnique = attacker.GetEffectiveTechnique();
            var (hitSteps, comboSteps, critSteps) = GetSteps(effectiveTechnique);
