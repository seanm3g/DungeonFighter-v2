...
- [x] **UI / Item Generation — affix + lab strip uses width:** Top “Real loot” row uses proportional affix columns (`*` with wider rarity label) so the matrix grows with the panel; lab **rarity %** + **tier %** sit in a fixed **340px** right column so they stay on-screen while extra width fills the affix table. Inner affix `ScrollViewer` horizontal scroll off so cells stretch with layout.
