...
- [ ] **Bugfix / Enemies — missing damaging actions warnings:** Some enemies log `has no damaging actions` at spawn because `GameData/Enemies.json` had `actions` (and other fields) incorrectly nested under `growthPerLevel`, leaving `EnemyData.Actions` empty after load. Normalize `Enemies.json` to the canonical shape so `EnemyLoader` / `EnemyActionManager` load at least one damaging action; add a unit test asserting `CreateEnemy("Goblin")` yields an action pool containing an `Attack` or `Spell`.
