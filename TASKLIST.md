...
- [x] **Combat / Unarmed — default Punch action:** When the hero has **no equipped weapon**, the default combo/action is **PUNCH** (100% damage, 100% speed) so Training Ground and any unarmed state has a consistent baseline attack. Code: `CharacterSerializer.EnsureActionPoolNotEmptyAfterRebuild`, `ComboSequenceManager.InitializeDefaultCombo`. Tests: `RebuildCharacterActionsComboTests`.
