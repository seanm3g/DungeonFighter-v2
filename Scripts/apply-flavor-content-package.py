...
LOCATION_DESCRIPTIONS: dict[str, list[str]] = {
    ...
    "Forest": [
        "Roots run in tight, even rings here, more deliberate than growth usually is.",
        "Wolves move through country like this in packs disciplined enough that old herders swear a cornered one still hesitates at a raised hand.",
        "Nobody's fed one by hand in longer than anyone living can prove.",
        "The quiet under these trees isn't empty. Somewhere this old, something is still being grieved, by someone who never stopped and never explained why.",
        "Bark has been carved here longer than any single hand could account for, one name laid over the last, further back than the trees should allow.",
        "Moss holds the shape of old fire pits in rings too even to be accident, cold now, banked by hands that stopped coming back.",
        "A hush moves through here that isn't caused by wind. Barbarians who've spent real time in this wood don't try to explain it to people who haven't.",
        "Old iron turns up half-buried under root and leaf, and it never quite rusts the way iron this old should.",
        "Fire pits sit cold in rings too neat to be one family's doing, banked by more than one set of hands.",
    ],
    "Generic": [
...
ROOM_CONTEXTS: dict[str, list[str]] = {
    "Forest/armory": [
        ...
        "A single roof shelters two separate racks of gear, worn in different styles, kept together anyway.",
    ],
    "Forest/boss": [
        "Whatever holds this grove has been here long enough that the trees have grown around it rather than through it.",
        "The roots here don't just reach — they circle, patient the way something with old blood and older training learns to be patient.",
        "The air changes at the center of this clearing, heavier, like a wolf deciding whether you're worth the trouble of a fight.",
    ],
    "Forest/chamber": [
        "The walls of this hollow are packed earth and root, grown, not built, though something clearly meant for it to hold a shape.",
        "Light filters down through a gap in the canopy that always seems to land in the same place, like something fixed it there on purpose.",
        "This room breathes with the forest around it, quiet enough to make you wonder who else has stood exactly here and gone equally still.",
    ],
    "Forest/kitchen": [
        "Ash from old cookfires is packed into the ground here, layered deep enough to mark generations sharing a single hearth.",
        "Dried herbs hang from a low beam of bent wood, hung by hands from two different houses, if the knot-work's any judge.",
        "A blackened hearth sits half-swallowed by roots, still warm though no one's fed it in years — the kind of warmth a shared roof leaves behind.",
    ],
    "Forest/library": [
        "Old script runs along the bark here in rows too straight for weather to have made.",
        "Some of the letters don't match themselves, shifting shape between one look and the next.",
        "Whatever kept this record trusted wood over parchment, and wood remembers longer than it should.",
        "Even the oldest parts of it, nobody living can read anymore.",
        "Stacked stones mark where something was written and rewritten, the older layer never quite gone, just built over.",
    ],
    "Forest/sanctum": [
        ...
        "Old banners hang past this ring, faded past their colors.",
        "They mark ground where two houses once agreed to share a roof, and stopped burning each other's holdings for it.",
    ],
    "Forest/shrine": [
        "Small offerings are tucked into the roots — coin, cloth, a single carved stone worn smooth from handling.",
        "A ring of stacked rocks marks the base of the oldest tree in the grove.",
        "They're set the way two hearths sit under one roof, close, deliberate, unwilling to be one fire.",
    ...
    "Forest/treasure": [
        ...
        "The kind of hurry that only comes from a fight nobody wrote down.",
        "A hollow in the oldest trunk holds more than it should, packed tight and gone dry with age, the way anything kept by more than one hand tends to.",
    ],
    "Crypt/armory": [
...
COMBAT_NARRATIVES: dict[str, list[str]] = {
    ...
    "playerTaunt_ice": [
        "\"{enemy}, everything moves slower once the cold gets into it. You're already behind.\"",
        "\"The frost doesn't care how tough you were somewhere warmer, {enemy}.\"",
        "\"{enemy}, I've outlasted worse than you standing still in weather like this.\"",
        "\"Nothing holds its edge out here for long, {enemy}. Let's see if you do.\"",
    ],
    "enemyTaunt_ice": [
        "\"You'll go stiff before you go down, {player}.\" {name}'s breath hangs between them, unbothered by the cold.",
        "\"{player}, the cold got here before you did, and it's not leaving either.\"",
        "\"Nothing that stops moving out here gets moving again, {player}.\"",
        "\"{player}, I don't feel the cold anymore. You will.\"",
    ],
    "playerTaunt_swamp": [
        "\"{enemy}, the mud doesn't let go easy. Neither will I.\"",
        "\"Everything sinks out here eventually, {enemy}. Might as well be you first.\"",
        "\"{enemy}, this ground's swallowed louder fights than this one and kept quiet about it.\"",
        "\"The water's still for a reason, {enemy}. It's waiting.\"",
    ],
    "enemyTaunt_swamp": [
        "\"You won't find solid ground before I find you, {player}.\" {name} doesn't sink the way {player} does.",
        "\"{player}, this swamp keeps what sinks into it. Everything does, eventually.\"",
        "\"The mud remembers every set of footprints it's swallowed, {player}. Yours are next.\"",
